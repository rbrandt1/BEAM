...
class RandomExplainer0:
    def __init__(self, model):
        pass;
    def __call__(self, gt_explanation, y_data):

        explanations = np.copy(gt_explanation)

        random = (np.random.uniform(size=gt_explanation.shape))*2  - 1
        alpha = .999

        explanations[gt_explanation != 0] = (1 - alpha) * explanations[gt_explanation != 0] + alpha * random[gt_explanation != 0]  

        return explanations


class RandomExplainer1:
    def __init__(self, model):
        pass;
    def __call__(self, gt_explanation, y_data):
...
class RandomExplainer5:
    def __init__(self, model):
        pass;
    def __call__(self, gt_explanation, y_data):
  
        explanations = np.copy(gt_explanation)
        explanations[gt_explanation > 0] = - gt_explanation[gt_explanation > 0]
        explanations[gt_explanation < 0] = - gt_explanation[gt_explanation < 0]
...
all_xai_methods =       [RandomExplainer0,RandomExplainer1,RandomExplainer2,RandomExplainer3,RandomExplainer4,RandomExplainer5] # XAI METRIC EVAL
all_xai_methods_names = ["new_0","new_1","new_2","new_3","new_4","new_5"] # XAI METRIC EVAL
...
def calc_metrics_for_XAI_method(Xai_method, xai_name, model, data, conceptid):
        ...
        explanations = explainer(data['test']['gt_explanation3D'], data['test']['y'])
        ...
        if settings['turn_gts_binary']:
            gt_explanation = gt_explanation.astype('float32')
            gt_explanation[gt_explanation > 0] = 1
            gt_explanation[gt_explanation < 0] = -1
...
def main():
    ...
    df.to_csv(r'table_results_True_'+str(settings['run_id'])+'_'+str(settings['turn_gts_binary'])+'.csv', index = True)
...
if __name__ == "__main__":
    ...
    for settings['turn_gts_binary'] in [False, True]:
        for settings['run_id'] in range(3):
            ...
            print("******************* RUN: ",settings['run_id'],"(turn_gts_binary:",settings['turn_gts_binary'],") *******************")
            main()
