...
def calc_metrics_for_XAI_method(Xai_method, xai_name, model, data, conceptid):
        ...
        if settings['saveSelectedImages']:
            ...
            saveImage(root_url+"paper_images/selected/True/"+str(settings['run_id'])+"/nocolormap_"+xai_name+"_"+str(class_idx)+".png", explanations[selected_idx])
...
def main():
    ...
    df.to_csv(root_url+'table_results_True_'+str(settings['run_id'])+'_'+str(settings['turn_gts_binary'])+'.csv', index = True)
