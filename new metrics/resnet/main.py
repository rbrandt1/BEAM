...
settings['num_examples_per_class'] = 1 
...
def calc_metrics_for_XAI_method(Xai_method, xai_name, model, data, conceptid):
        ...
        explainer = Xai_method(model,batch_size=8)
        ...
        if not settings['calc_metrics']:
            print("Not calculating metrics")
        else:
        ...
        if settings['saveSelectedImages']:
            ...
            saveImage("./paper_images/selected/True/"+str(settings['run_id'])+"/nocolormap_"+xai_name+"_"+str(class_idx)+".png", explanations[selected_idx])
            saveImageColomap("./paper_images/selected/True/"+str(settings['run_id'])+"/"+xai_name+"_"+str(class_idx)+".png", explanations[selected_idx])
            saveImageColomap("./paper_images/selected/True/"+str(settings['run_id'])+"/gt2d_"+str(class_idx)+".png", gt_explanation[selected_idx])
            saveImage("./paper_images/selected/True/"+str(settings['run_id'])+"/input_"+str(class_idx)+".png", data['test']['x'][selected_idx])

            if settings['saveAllImages']:
                
                saveImages("./paper_images/explanations/True/"+str(settings['run_id'])+"/",explanations,conceptid, True,data,"_"+xai_name) # (path,images,conceptid, applyColormap,data,postfix="")
                saveImages("./paper_images/input/True/"+str(settings['run_id'])+"/",data['test']['x'],conceptid, False,data)
                saveImages("./paper_images/input/True/"+str(settings['run_id'])+"/",gt_explanation,conceptid, True,data,"_gt")
...
def main():
    ...
    df.to_csv('table_results_True_'+str(settings['run_id'])+'.csv', index = True)
    ...
    df_time.to_csv('table_times_True_'+str(settings['run_id'])+'.csv', index = True)
...
if __name__ == "__main__":
    ...
    all_xai_methods = [GradCAM,GradCAMPP,Saliency,DeconvNet, GradientInput, GuidedBackprop, IntegratedGradients, SmoothGrad, SquareGrad,VarGrad,KernelShap,Lime,Occlusion,Rise]
    all_xai_methods_names = ["GradCAM","GradCAMPP","Saliency","DeconvNet", "GradientInput", "GuidedBackprop", "IntegratedGradients", "SmoothGrad", "SquareGrad","VarGrad","KernelShap","Lime","Occlusion","Rise"]
          
    all_xai_metrics = [Deletion,Insertion] 
    all_xai_metrics_names = ["Deletion","Insertion"]


    for settings['run_id'] in [2]:
